...
int main(int argc, char *argv[]) {
...
#endif
    Log::debug("KwiK UI - Vulkan 渲染示例\n");
    // ── 1. 创建窗口 ────────────────────────────────────────────
    auto window = std::make_unique<PlatformWindowWin32>();
    if (!window || !window->Create("KwiK UI - Vulkan", 800, 600)) {
        std::print("错误: 创建窗口失败\n");
        return -1;
    }
    window->Show();
    ...
    while (running) {
        ...
        Log::debug("[MAIN] frame {}\n", frameCount);
        window->PollEvents();
        // Log::debug("[MAIN] events polled\n");
        ...
        std::print("[MAIN] submit={} depth={}\n", "ok", renderThread.commandQueue().depth());
        frameCount++;
        ...
        std::print("[MAIN] loop exited, frameCount={}\n", frameCount);
    }
    std::print("\n程序正常退出\n");
    return 0;
}