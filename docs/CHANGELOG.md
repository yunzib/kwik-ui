...
- 新增 KwikRuntime：一个窗口一棵 UI 树的全部运行状态（渲染线程/JS 上下文/
  树/层树/动画/事件路由各自独立）；Application 改为运行时集合 +
  createRuntime 创建 + 多树主循环（旧单窗构造移除，示例入口同步更新）
- 层树管理器与动画引擎去全局单例，改为每棵树一份：组件经根节点服务槽
  沿父链上行获取本树实例（LayerStack 拆独立模块；动画句柄纯 token 化，
  组操作并入引擎方法——消除句柄与引擎的相互引用）
- 纹理管理器按渲染后端分域：创建/销毁显式携带目标后端，多窗口纹理互不
  串扰；Image/Video 组件经服务槽路由本树后端
- 双窗口验证：example 新增 dual 模式（主窗 layer + 副窗 glass 并行运行，
  冒烟 60 帧零错误）；37 示例回归与 Vulkan 验证层全绿

# 0.0.0 — 2026-09-21
### 修复
- TextArea 无析构清理光标闪烁定时器：HMR/关窗树重建后回调 [this] 悬垂，
  下个 tick 触发即崩（~Input 同款兜底补齐；Input/Chart 审计确认已有）
- CoreTimer 补 stopAll：HMR 前清场兜底（正规防线为组件析构 clear）

### 重构
- 程序退出和界面热重载两处各自重复写的清理步骤（停止动画、清空定时器、
  清空弹层列表）收拢为一个公共方法，执行的先后顺序及原因集中注释在
  该方法内——此前两处各写一份，将来改一处漏一处会引入销毁顺序问题
