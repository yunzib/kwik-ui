...
```
## 2.2 特点
- 每次写入都触发整树重建（form.key = val → rebuildTree()），非 Virtual DOM diff
- State 是 QuickJS exotic 对象，通过 set_property hook 拦截写入并回调 C++ render_callback
- form.key = ""（空串）也会触发重建
- State 方法（.update()）不触发重建
- 只有函数式导出会响应 State 变化：export default () => View(...) 而非 export default View(...)
...
### 与 PropBus 协作
- setProp 在有绑定的组件上会间接更新 State：
setProp("userName", "value", "新值");
- // → binding_->setString → form.name = "新值" → rebuild
getProp / setProp 本身不触发 rebuild，适合高频操作。
