...
module kwik.engine.color_parser;
...
import kwik.core.types;
    ...
    Color parseColor(const std::string& str) {
        ...
        if (s.size() == 4 && s[0] == '#') {
            return {
                static_cast<uint8_t>(parseHex(s[1]) * 17),
                static_cast<uint8_t>(parseHex(s[2]) * 17),
                static_cast<uint8_t>(parseHex(s[3]) * 17),
                255
            };
        }
        ...
        if (s.size() == 7 && s[0] == '#') {
            return {
                static_cast<uint8_t>((parseHex(s[1]) << 4) | parseHex(s[2])),
        ...
        if (s.substr(0, 5) == "rgba(") {
            ...
            return {static_cast<uint8_t>(r), static_cast<uint8_t>(g), 
                    static_cast<uint8_t>(b), static_cast<uint8_t>(a * 255)};
        }
        ...
        if (s.substr(0, 4) == "rgb(") {
            ...
            return {static_cast<uint8_t>(nextNum()), 
                    static_cast<uint8_t>(nextNum()),
                    static_cast<uint8_t>(nextNum()), 255};
        }
