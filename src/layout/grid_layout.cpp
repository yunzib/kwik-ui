...
void GridLayout::onLayout() {
    int cols = std::max(1, props.gridCols);
    int rows = std::max(1, props.gridRows);
    ...
    float cellW = (contentW - props.columnGap * (cols - 1)) / cols;
    float cellH = (contentH - props.rowGap * (rows - 1)) / rows;
    for (auto &child : children) {
        ...
        float cx = contentX + c * (cellW + props.columnGap) + child->props.margin.left;
        float cy = contentY + r * (cellH + props.rowGap) + child->props.margin.top;
        float cw = cellW * cs + props.columnGap * (cs - 1) - child->props.margin.horizontal();
        float ch = cellH * rs + props.rowGap * (rs - 1) - child->props.margin.vertical();
        if (cw < 0) cw = 0;
        if (ch < 0) ch = 0;
