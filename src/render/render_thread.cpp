...
bool RenderThread::start() {
    ...
    if (state_ != RenderThreadState::Stopped && state_ != RenderThreadState::Error) {
        return false;
    }
...
void RenderThread::threadMain() {
    ...
    try {
        ...
        // 通知启动完成
        if (config_.callbacks.onStarted) {
            config_.callbacks.onStarted();
        }
        ...
        while (true) {
            ...
            {
                std::lock_guard<std::mutex> lock(stateMutex_);
                if (state_ == RenderThreadState::Stopping) {
                    break;
                }
            }
        ...
        // 通知错误回调
        if (config_.callbacks.onError) {
            config_.callbacks.onError(e.what());
        }
    ...
    // 通知停止回调
    if (config_.callbacks.onStopped) {
        config_.callbacks.onStopped();
    }
}
    ...
    switch (event.type) {
        case Event::Type::WindowResize: {
            ...
            if (event.width > 0 && event.height > 0) {
                ...
                // 通知主线程
                if (config_.callbacks.onResize) {
                    config_.callbacks.onResize(width_, height_);
                }
            }
            break;
        }
...
void RenderThread::updateFrameStats(std::chrono::high_resolution_clock::time_point frameStartTime) {
    ...
    if (frameStats_.totalFrames == 1) {
        ...
        frameStats_.averageFrameTimeMs = 
            0.9f * frameStats_.averageFrameTimeMs + 0.1f * frameTime;
    }
    
    // 更新最大队列深度
    size_t currentDepth = commandQueue_.depth();
    if (currentDepth > frameStats_.maxQueueDepth) {
        frameStats_.maxQueueDepth = currentDepth;
    }
